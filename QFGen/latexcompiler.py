import os
import subprocess
import tempfile
import shutil
import time
from tqdm import tqdm
import customtkinter as ctk
from threading import Thread
from queue import Queue

class CompilationError(Exception):
        """Exception levée en cas d'erreur de compilation."""
        pass

class LaTeXCompiler:
    """
    Classe permettant de gérer des documents LaTeX, incluant la lecture et la sauvegarde des documents,
    des manipulations de base, la compilation avec LuaLaTeX ou avec une double compilation pour les sommaires,
    et l'ouverture du fichier PDF résultant dans Microsoft Edge.
    """

    def __init__(self, document_paths,auxiliary_suppressor_on=False):
        """
        Initialise l'instance avec une liste de chemins vers les documents LaTeX.
        """
        self.error_code=""
        self.auxiliary_suppressor_on=auxiliary_suppressor_on
        self.documents = document_paths  # Liste des chemins des documents LaTeX
        self.data_compilation_times="latexcompiler_compilation_times.txt"
        self.compilation_times = self._load_compilation_times()


    def _load_compilation_times(self):
        """Charge les temps de compilation à partir du fichier."""
        if not os.path.exists(self.data_compilation_times):
            return []

        with open(self.data_compilation_times, 'r') as f:
            times = [float(line.strip()) for line in f.readlines()]
        
        return times
    def _save_compilation_time(self, duration):
        """Enregistre le temps de compilation dans le fichier."""
        self.compilation_times.append(duration)
        if len(self.compilation_times) > 20:
            self.compilation_times.pop(0)  # Garder seulement les 20 derniers temps

        with open(self.data_compilation_times, 'w') as f:
            for time in self.compilation_times:
                f.write(f"{time}\n")

    def _get_average_compilation_time(self):
        """Calcule le temps moyen de compilation à partir des 20 derniers enregistrements."""
        if not self.compilation_times:
            return 100  # Valeur par défaut si aucune donnée
        return sum(self.compilation_times) / len(self.compilation_times)
    
    def backup_documents(self, backup_dir):
        """
        Copie et sauvegarde les documents dans un répertoire spécifié.
        """
        os.makedirs(backup_dir, exist_ok=True)
        for doc in self.documents:
            shutil.copy(doc, os.path.join(backup_dir, os.path.basename(doc)))

    def manipulate_document(self, document_path, manipulation_func):
        """
        Applique une fonction de manipulation sur le contenu d'un document LaTeX.
        """
        try:
            with open(document_path, 'r', encoding='utf-8') as file:
                content = file.read()

            content = manipulation_func(content)

            with open(document_path, 'w', encoding='utf-8') as file:
                file.write(content)
        except Exception as e:
            self.error_code="Erreur lors de la manipulation du document : {}".format(e)
            return False
        return True
    def destroy_auxiliary_files(self, pdf_path):
        """
        Supprime les fichiers auxiliaires générés par la compilation LaTeX pour améliorer l'expérience de compilation automatique.
        """
        success=False
        # Définir les extensions de fichiers auxiliaires à supprimer
        auxiliary_extensions = ['aux', 'log', 'out', 'toc', 'lof', 'lot', 'fls', 'fdb_latexmk', 'synctex.gz','comp','voc']

        # Obtenir le chemin de base (sans extension) du fichier PDF
        base_path = os.path.splitext(pdf_path)[0]
        base_path.replace(".pdf","")
        tex_base_path = base_path.replace('/pdf/','/tex/')
        print(f"base path : {base_path}")

        # Parcourir les extensions et supprimer les fichiers correspondants
        for ext in auxiliary_extensions:
            file_to_remove = f"{base_path}.{ext}"
            tex_file_to_remove=f"{tex_base_path}.{ext}"
            if os.path.exists(file_to_remove):
                try:
                    os.remove(file_to_remove)
                    #os.remove(tex_file_to_remove)
                    print(f"Supprimé : {file_to_remove}")
                except Exception as e:
                    print(f"Erreur lors de la suppression de {file_to_remove}: {e}")
            '''else:
                print(f"Fichier non présent : {file_to_remove}")'''
            if os.path.exists(tex_file_to_remove):
                try:
                    #os.remove(file_to_remove)
                    os.remove(tex_file_to_remove)
                    print(f"Supprimé : {tex_file_to_remove}")
                except Exception as e:
                    print(f"Erreur lors de la suppression de {tex_file_to_remove}: {e}")
            '''else:
                print(f"Fichier non présent : {tex_file_to_remove}")'''
        dir_path,file_name = os.path.split(base_path)
        # Ajout de "bfpoints_" avant le nom du fichier
        modified_file_name = f"bfpoints_{file_name}"
        #print(f"modified_file_name_bfpoints : {modified_file_name}")
        # Reconstruction du chemin complet
        points_file_suppr = os.path.join(dir_path, modified_file_name)
        if os.path.exists(f'{points_file_suppr}.tex'):
                try:
                    #points_file_suppr = f'bfpoints_{base_path}.tex'
                    os.remove(f'{points_file_suppr}.tex')
                    print(f"Supprimé : {f'{points_file_suppr}.tex'}")
                except Exception as e:
                    print(f"Erreur lors de la suppression de {points_file_suppr}.tex : {e}")
        '''else:
            print(f"Erreur lors de la suppression de {points_file_suppr}.tex")'''
        success=True
        return success
        
    def compile_latex_errors_terminal_output(self, document_path, output_directory=None, double_compile=False):
        """
        Compile le document LaTeX en utilisant LuaLaTeX ou une double compilation si nécessaire.
        Affiche une barre de progression pendant la compilation.
        """
        compiler = "lualatex" # ou "pdflatex" 
        if output_directory:
            pdf_output_path = os.path.join(output_directory, os.path.splitext(os.path.basename(document_path))[0] + ".pdf")
            compile_command = [compiler, "-output-directory=" + output_directory, document_path]
        else:
            pdf_output_path = os.path.splitext(document_path)[0] + ".pdf"
            compile_command = [compiler, document_path]

        average_time = self._get_average_compilation_time()

        try:
            with tempfile.NamedTemporaryFile(mode='w+', delete=False, encoding='utf-8') as tempf:
                tempf_path = tempf.name
                with tqdm(total=average_time, desc="Compilation en cours", unit="s") as pbar:
                    start_time = time.time()
                    process = subprocess.Popen(compile_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8')

                    while True:
                        output = process.stdout.readline()
                        if output == '' and process.poll() is not None:
                            break
                        if output:
                            tempf.write(output)
                            tempf.flush()
                            elapsed_time = time.time() - start_time
                            pbar.update(min(elapsed_time - pbar.n, pbar.total - pbar.n))  # Mise à jour de la barre de progression

                    # Vérifie le code de retour du processus pour les erreurs
                    if process.returncode != 0:
                        raise CompilationError("Erreur de compilation : {}".format(process.returncode))

                    if double_compile:
                        tempf.seek(0)
                        tempf.truncate()
                        start_time = time.time()
                        process = subprocess.Popen(compile_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8')
                        while True:
                            output = process.stdout.readline()
                            if output == '' and process.poll() is not None:
                                break
                            if output:
                                tempf.write(output)
                                tempf.flush()
                                elapsed_time = time.time() - start_time
                                pbar.update(min(elapsed_time - pbar.n, pbar.total - pbar.n))  # Mise à jour de la barre de progression

                        if process.returncode != 0:
                            raise CompilationError("Erreur de compilation : {}".format(process.returncode))

            total_duration = time.time() - start_time
            self._save_compilation_time(total_duration)
            self.last_duration = total_duration
            self.pdf_output_path = pdf_output_path
            return True, pdf_output_path
        except CompilationError as e:
            self.error_code = str(e)
            with open(tempf_path, 'r', encoding='utf-8') as tempf:
                log_lines = tempf.readlines()[-20:]
                print("".join(log_lines))
            return False, None
        finally:
            # Supprimer le fichier temporaire
            if os.path.exists(tempf_path):
                os.remove(tempf_path)



    def compile_latex_errors_graphical_output(self, document_path, output_directory=None, double_compile=False):
        """
        Compile le document LaTeX en utilisant LuaLaTeX ou une double compilation si nécessaire.
        Affiche une barre de progression dans une fenêtre modale pendant la compilation.
        """
        compiler = "lualatex"  # ou "lualatex"

        print("new compile latex triggered")
        if output_directory:
            pdf_output_path = os.path.join(output_directory, os.path.splitext(os.path.basename(document_path))[0] + ".pdf")
            compile_command = [compiler, "-output-directory=" + output_directory, document_path]
        else:
            pdf_output_path = os.path.splitext(document_path)[0] + ".pdf"
            compile_command = [compiler, document_path]

        average_time = self._get_average_compilation_time()

        # Création d'une fenêtre indépendante
        progress_window = ctk.CTkToplevel()
        progress_window.title("Compilation en cours")
        progress_window.geometry("400x150")
        progress_window.attributes("-topmost", True)
        # Barre de progression
        progress_bar = ctk.CTkProgressBar(progress_window, width=300)
        progress_bar.pack(pady=20)
        progress_bar.set(0)  # Initialiser la progression à 0%

        # Label pour le statut
        status_label = ctk.CTkLabel(progress_window, text="Compilation en cours...")
        status_label.pack(pady=10)
        # Queue pour récupérer le résultat
        result_queue = Queue()
        def update_progress_bar(process, tempf, start_time, pbar_total):
            """Met à jour la barre de progression pendant la compilation."""
            mcount=0
            while True and mcount<1000000:
                mcount+=1
                output = process.stdout.readline()
                if output == '' and process.poll() is not None:
                    break
                if output:
                    tempf.write(output)
                    tempf.flush()
                    elapsed_time = time.time() - start_time
                    progress = min(elapsed_time / pbar_total, 1.0)
                    progress_bar.set(progress)
                    progress_window.update()

        def run_compilation():
            """Exécute la compilation avec suivi."""
            try:
                with tempfile.NamedTemporaryFile(mode='w+', delete=False) as tempf:
                    start_time = time.time()
                    process = subprocess.Popen(compile_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)

                    update_progress_bar(process, tempf, start_time, average_time)

                    if process.returncode != 0:
                        raise subprocess.CalledProcessError(process.returncode, compile_command)

                    # Double compilation si nécessaire
                    if double_compile:
                        tempf.seek(0)
                        tempf.truncate()
                        start_time = time.time()
                        process = subprocess.Popen(compile_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
                        update_progress_bar(process, tempf, start_time, average_time)

                        if process.returncode != 0:
                            raise subprocess.CalledProcessError(process.returncode, compile_command)

                total_duration = time.time() - start_time
                self._save_compilation_time(total_duration)
                self.last_duration = total_duration
                result_queue.put((True, pdf_output_path))  # Retourner le succès et le chemin
                status_label.configure(text="Compilation terminée !")
                progress_bar.set(1.0)
                time.sleep(1)
            except subprocess.CalledProcessError as e:
                self.error_code = f"Erreur de compilation : {e}"
                result_queue.put((False, None))
                status_label.configure(text="Erreur de compilation !")
            finally:
                progress_window.destroy()
                if os.path.exists(tempf.name):
                    os.remove(tempf.name)

        # Lancer la compilation dans un thread pour ne pas bloquer l'interface
        Thread(target=run_compilation).start()
        # Attendre la fin du thread et récupérer le résultat
        progress_window.wait_window()  # Attend la fermeture de la fenêtre modale
        success, result = result_queue.get()  # Récupérer la valeur de retour
        return success, result
    
    def compile_latex_last_stable(self, document_path, output_directory=None, double_compile=False):
        """
        Compile le document LaTeX en utilisant LuaLaTeX ou une double compilation si nécessaire.
        Affiche une barre de progression pendant la compilation.
        """
        compiler = "pdflatex"  # ou "lualatex"
        if output_directory:
            pdf_output_path = os.path.join(output_directory, os.path.splitext(os.path.basename(document_path))[0] + ".pdf")
            compile_command = [compiler, "-output-directory=" + output_directory, document_path]
        else:
            pdf_output_path = os.path.splitext(document_path)[0] + ".pdf"
            compile_command = [compiler, document_path]

        average_time = self._get_average_compilation_time()

        try:
            with tempfile.NamedTemporaryFile(mode='w+', delete=False) as tempf:
                with tqdm(total=average_time, desc="Compilation en cours", unit="s") as pbar:
                    start_time = time.time()
                    process = subprocess.Popen(compile_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)

                    while True:
                        output = process.stdout.readline()
                        if output == '' and process.poll() is not None:
                            break
                        if output:
                            tempf.write(output)
                            tempf.flush()
                            elapsed_time = time.time() - start_time
                            pbar.update(min(elapsed_time - pbar.n, pbar.total - pbar.n))  # Mise à jour de la barre de progression

                    # Vérifie le code de retour du processus pour les erreurs
                    if process.returncode != 0:
                        raise subprocess.CalledProcessError(process.returncode, compile_command)

                    if double_compile:
                        tempf.seek(0)
                        tempf.truncate()
                        start_time = time.time()
                        process = subprocess.Popen(compile_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
                        while True:
                            output = process.stdout.readline()
                            if output == '' and process.poll() is not None:
                                break
                            if output:
                                tempf.write(output)
                                tempf.flush()
                                elapsed_time = time.time() - start_time
                                pbar.update(min(elapsed_time - pbar.n, pbar.total - pbar.n))  # Mise à jour de la barre de progression

                        if process.returncode != 0:
                            raise subprocess.CalledProcessError(process.returncode, compile_command)

            total_duration = time.time() - start_time
            self._save_compilation_time(total_duration)
            self.last_duration=total_duration
            return True, pdf_output_path
        except subprocess.CalledProcessError as e:
            self.error_code = "Erreur de compilation : {}".format(e)
            with open(tempf.name, 'r') as tempf:
                log_lines = tempf.readlines()[-20:]
                print("".join(log_lines))
            return False, None
        finally:
            # Supprimer le fichier temporaire
            if os.path.exists(tempf.name):
                os.remove(tempf.name)
    def compile_latex_old(self, document_path, output_directory=None, double_compile=False):
        """
        Compile le document LaTeX en utilisant LuaLaTeX ou une double compilation si nécessaire.
        """
        compiler = "pdflatex"#"lualatex"
        if output_directory:
            pdf_output_path = os.path.join(output_directory, os.path.splitext(os.path.basename(document_path))[0] + ".pdf")
            compile_command = [compiler, "-output-directory=" + output_directory, document_path]
        else:
            pdf_output_path = os.path.splitext(document_path)[0] + ".pdf"
            compile_command = [compiler, document_path]
        
        try:
            subprocess.run(compile_command, check=True)
            if double_compile:  # Double compilation pour les documents nécessitant un sommaire
                subprocess.run(compile_command, check=True)
        except subprocess.CalledProcessError as e:
            self.error_code = "Erreur de compilation : {}".format(e)
            return False, None
        return True, pdf_output_path

    def open_pdf_in_edge(self, pdf_path):
        """
        Tente d'ouvrir le fichier PDF dans le navigateur par défaut pour les fichiers PDF.
        """
        try:
            if not os.path.exists(pdf_path):
                self.error_code = "Le fichier PDF n'existe pas."
                return False

            os.startfile(pdf_path)
        except Exception as e:
            self.error_code = "Erreur lors de l'ouverture du fichier PDF : {}".format(e)
            return False
        return True

    def compile_and_open(self, document_path, output_directory=None, double_compile=False):
        """
        Compile le fichier LaTeX et ouvre le PDF résultant.
        """
        success, pdf_path = self.compile_latex_errors_graphical_output(document_path, output_directory, double_compile)
        if not success:
            print("Échec de la compilation :", self.error_code)
            return False

        if not self.open_pdf_in_edge(pdf_path):
            print("Échec de l'ouverture du PDF :", self.error_code)
            return False

        print(f"Compilation et ouverture réussies - durée : {self.last_duration}")
        if self.auxiliary_suppressor_on==True:
            auxiliary_destroyed=self.destroy_auxiliary_files(pdf_path)
            if auxiliary_destroyed:
                print("Les fichiers auxiliaires ont bien été supprimés.")
            else:
                print("Les fichiers auxiliaires n'ont pas pu être supprimés.")
        return True
    
    def compile_and_return_pdf_path(self, document_path, output_directory=None, double_compile=False):
        """
        Compile le fichier LaTeX et retourne le chemin du PDF résultant.
        """
        if output_directory:
            success, pdf_path = self.compile_latex_errors_graphical_output(document_path, output_directory, double_compile=False)
        else:
            success, pdf_path = self.compile_latex_errors_graphical_output(document_path, double_compile=False)#, output_directory
        if not success:
            print("Échec de la compilation :", self.error_code)
            return None
        auxiliary_destroyed=self.destroy_auxiliary_files(pdf_path)
        if auxiliary_destroyed:
            print("Les fichiers auxiliaires ont bien été supprimés.")
        return pdf_path