...
if __name__=="__main__":
    ...
    # Some option list
    values = ["86 : BROSSOLETTE","tkinter","customtkinter","widgets",
            "options","menu","combobox","dropdown","search"]

    frame=customtkinter.CTkFrame(root,width=300)
    frame.pack(fill="x", padx=10, pady=10)
    #entry = customtkinter.CTkEntry(frame, width=240)
    #entry.pack(fill="x", padx=10, pady=10,side="left")
    entry = customtkinter.CTkComboBox(frame, width=240)
    entry.pack(fill="x", padx=10, pady=10,side="left")
    label=customtkinter.CTkLabel(frame, width=240,text="")
    label.pack(fill="x", padx=10, pady=10,side="right")
    # method to insert the chosen option from the autocomplete
    def insert_method(e,label):
        #entry.delete(0, 'end')
        try:
            entry_text=e.split(" :")[0]
            label.configure(text=e.split(" :")[1])
        except:
            entry_text=e
        #entry.insert(0, entry_text)
        entry.set(entry_text)

    dd=CTkScrollableDropdown(entry, values=values, command=lambda e: insert_method(e,label=label),autocomplete=True) # Using autocomplete
    root.mainloop()