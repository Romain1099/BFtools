...
class ExoAbstractor:
    def __init__(self,texte:str=""):
        ...
        self.modified_list=[]
        self.ai_api = AI_API.lower()

        # Configuration selon l'API choisie
        if self.ai_api == 'claude':
            self.claude_api_key = os.environ.get('ANTHROPIC_API_KEY', read_api_key('claude').strip().strip('"'))
            self.claude_client = AsyncAnthropic(api_key=self.claude_api_key)
        else:  # openai par défaut
            self.openai_api_key = read_api_key().strip().strip('"')
    ...
    async def generate_questions(self,text:str)->str:
        ...
        while attempts < max_attempts:
            try:
                ...
                print(f"Error with {self.ai_api}: {e}. Attempt {attempts}/{max_attempts}")
                time.sleep(2)
            finally:
                attempts += 1
        if attempts > max_attempts:
            print(f"Failed to generate questions with {self.ai_api} after several attempts. Please try again later.")

        #print(code)
        return code

    async def abstract_AI(self,text,stream_wanted=True):
        """Fonction façade qui choisit l'API appropriée"""
        if self.ai_api == 'claude':
            return await self._abstract_AI_claude(text, stream_wanted)
        else:
            return await self._abstract_AI_openai(text, stream_wanted)

    async def _abstract_AI_openai(self, text, stream_wanted=True):
        """Implémentation pour OpenAI"""
        response = openai.ChatCompletion.create(
            model="gpt-4",  # ou gpt-4-turbo-preview, gpt-4o
            messages=[
        ...
        if stream_wanted:
            ...
            tex_code.append(response.choices[0].message.content)
        return ''.join(tex_code)

    async def _abstract_AI_claude(self, text, stream_wanted=True):
        """Implémentation pour Claude API"""
        try:
            if stream_wanted:
                # Version avec streaming
                tex_code = []
                async with self.claude_client.messages.stream(
                    model="claude-sonnet-4-5-20250929-thinking",  # ou claude-3-opus-20240229
                    max_tokens=8000,
            ...
            print(f"Erreur avec l'API Claude: {e}")
            raise
