...
class JsonQuestionLoader(ctk.CTkToplevel):
    ...
    def init_widgets(self):
        ...
        # TextEntry 1 : fichiers JSON avec scrollable dropdown
        self.combo_folders = ctk.CTkComboBox(self.selection_frame, width=300)
        self.combo_folders.pack(side="left", fill="x", expand=True, padx=5)

        # TextEntry 2 : questions dans le fichier sélectionné avec scrollable dropdown
        self.combo_questions = ctk.CTkComboBox(self.selection_frame, width=300)
        ...
        self.label_question_preview = Text(
            self.preview_frame,
            wrap="word",
    ...
    def select_first_unused_question(self,first_file):
        ...
        for index, question in enumerate(questions):
            ...
            if unique_id not in done_list:
                ...
                self.on_combobox_2_selected(question_text)
                break
    ...
    def on_combobox_0_selected(self, selected_base_folder):
        ...
        self.dropdown_filenames = CTkScrollableDropdown(
                self.combo_filenames,
                values=file_display,
                command=self.on_combobox_1_selected,
                autocomplete=True
            )
        
        self.combo_filenames.set(file_display[0])
    ...
    def on_combobox_1_selected(self, selected_file_display):
        ...
        self.combo_filenames.set(selected_file_display)
        ...
        self.dropdown_questions = CTkScrollableDropdown(
            self.combo_questions,
            values=question_display,
            command=self.on_combobox_2_selected,
            autocomplete=True
        )
        self.select_first_unused_question(selected_file)
    ...
    def on_combobox_2_selected(self, selected_question):
        ...
        if not selected_question:
            return
        self.combo_questions.set(selected_question)
        ...
        q_dict = self.questions_dict[selected_file]["questions_list"][question_index]
        formatted_preview = self.make_preview_from_json_dumps(q_dict)
        ...
        self.label_question_preview.insert("1.0", formatted_preview)
