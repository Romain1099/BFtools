...
async function handleScoreChange(event) {
    ...
    try {
        ...
        if (!result.success) {
            showError('Erreur lors de la sauvegarde du score');
            return;
        }
...
/**
 * Démarre le heartbeat pour maintenir le serveur actif
 */
function startHeartbeat() {
    ...
    setInterval(async () => {
        try {
            ...
            // Le serveur est probablement arrêté
            console.log('Heartbeat failed - server may be stopped');
        }
    }, 5000);  // 5 secondes

    // Envoyer un signal quand la page se ferme
    window.addEventListener('beforeunload', () => {
        // Optionnel : on pourrait envoyer un signal de déconnexion
        // mais le heartbeat qui s'arrête est suffisant
    });
}
...
async function importClass() {
    // TODO: Implémenter l'import de fichier CSV
    showError('Fonction d\'import en cours de développement');
}
...
function hasBeenQuestionedInCycle(student) {
    for (const key in student) {
        if (isDateField(key) && student[key] !== null && student[key] !== undefined) {
            return true;
        }
    }
    return false;
...
async function resetCycle() {
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification('Nouveau cycle démarré ! Tous les élèves sont à nouveau disponibles.');
        } else {
