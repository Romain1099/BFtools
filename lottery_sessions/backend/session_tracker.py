...
class SessionTracker:
    ...
    def filter_students_by_history(self, students: List[Dict], date: Optional[str] = None) -> List[Dict]:
        ...
        for student in students:
            ...
            for key in student.keys():
                ...
                if self._is_date_field(key) and value is not None and value != '' and value != 'ABS':
    ...
    def _get_current_cycle_fields(self, student: Dict) -> List[str]:
        """
        Retourne toutes les dates (cycle en cours, plus de logique NEW_CYCLE)
        ...
        """
        all_fields = list(student.keys())
        # Retourner simplement toutes les dates (plus de logique NEW_CYCLE)
        return [f for f in all_fields if self._is_date_field(f)]
    ...
    def is_cycle_complete(self, students: List[Dict]) -> bool:
        """
        Vérifie si tous les étudiants ont été interrogés dans le cycle actuel

        Args:
            students: Liste des étudiants

        Returns:
            True si tous les étudiants ont été interrogés au moins une fois dans le cycle actuel
        """
        for student in students:
            has_been_questioned = False

            # Obtenir les dates du cycle actuel
            cycle_fields = self._get_current_cycle_fields(student)

            # Vérifier s'il a été interrogé dans le cycle actuel
            for field in cycle_fields:
                value = student.get(field)
                # Considéré comme interrogé si: a un SCORE réel (0-3), PAS "ABS"
                if value is not None and value != '' and value != 'ABS':
                    has_been_questioned = True
                    break

            # Si au moins un étudiant n'a jamais été interrogé dans ce cycle, le cycle n'est pas complet
            if not has_been_questioned:
                return False
    ...
    def get_cycle_progress(self, students: List[Dict]) -> Dict:
        """
        Retourne les statistiques de progression du cycle actuel

        Args:
            students: Liste des étudiants

        Returns:
            Dictionnaire avec les stats du cycle actuel
        """
        total_students = len(students)
